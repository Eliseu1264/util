...
const
  ...
  UndoNames: array[TUndoName] of string =
    ('', 'digitação', 'apagar', 'mover', 'cortar', 'colar');
...
//  SaveResp := Pergunta(Format('Salvar alterações em %s?', [FFileName]));
  try
    if SaveResp = 'S' then FileSave(Self);
    if SaveResp = 'N' then Exit;
  finally
    FocusEditor;
  end;
end;
...
begin
  ...
  StatusBar.Panels[0].Text := Format('Lin: %3d  Col: %3d',
...
begin
  MessageDlg(Format('Texto "%s" não encontrado.', [FindText]), mtWarning,
    [mbOk], 0);
end;
