...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        C170.COD_ENQ := FieldByName('COD_ENQ').AsString; // Código de enquadramento legal do IPI, conforme tabela indicada no item 4.5.3.
        C170.CST_IPI := StrToCstIpi(FieldByName('CST_IPI').AsString); // Código da Situação Tributária referente ao IPI, conforme a Tabela indicada no item 4.3.2.
        C170.VL_BC_IPI := IfThen(ParamValido(853, 'SIM'), FieldByName('VL_BC_IPI').AsFloat, 0); // Valor da base de cálculo do IPI
        C170.ALIQ_IPI := IfThen(ParamValido(853, 'SIM'), FieldByName('ALIQ_IPI').AsFloat, 0); // Alíquota do IPI
        C170.VL_IPI := IfThen(ParamValido(853, 'SIM'), FieldByName('VL_IPI').AsFloat, 0); // Valor do IPI creditado/debitado
      end;

      C170.CST_PIS := StrToCstPis(FieldByName('CST_PIS').AsString); // Código da Situação Tributária referente ao PIS.
      C170.VL_BC_PIS := FieldByName('VL_BC_PIS').AsFloat; // Valor da base de cálculo do PIS
      C170.ALIQ_PIS_PERC := FieldByName('ALIQ_PIS_PERC').AsFloat;
      C170.VL_PIS := C170.VL_BC_PIS * C170.ALIQ_PIS_PERC / 100;

      C170.CST_COFINS := StrToCstCofins(FieldByName('CST_COFINS').AsString); // Código da Situação Tributária referente ao COFINS.
      C170.VL_BC_COFINS := FieldByName('VL_BC_COFINS').AsFloat; // Valor da base de cálculo da COFINS
...
//      D100.VL_DESC := FieldByName('VL_DESC').AsFloat; //Valor total do desconto
      ...
      D100.COD_CTA := COD_CTA_PRODUTO.Text; //Código da conta analitica contabil debitada/creditada
